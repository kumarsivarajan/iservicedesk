...
function deleteGroup(index) {
	...
	// add back pie/line charts
    if(groups_arr.length == 1) {
	  document.EditView.chart_type.options[3] = chartTypesHolder.pop();
	  document.EditView.chart_type.options[4] = chartTypesHolder.pop();
    }
	reload_columns('add');
}
...
function addFilterInputDate(row,filter) {
	...
	img_element.setAttribute('src',image_path+'/jscalendar.gif'); 
...
function addGroupByFromButton(group) {
  addGroupBy(group);
  // remove charts that don't make sense for multiple groupbys
  if(groups_arr.length > 1 && document.EditView.chart_type.options.length > 3) {
	  chartTypesHolder.push(document.EditView.chart_type.options[3]);
	  document.EditView.chart_type.options[3] = null;
	  chartTypesHolder.push(document.EditView.chart_type.options[3]);
	  document.EditView.chart_type.options[3] = null;
  }
  reload_columns('add');
...
function fill_form(type) {
  ...
  // we want an export of csv:
	...
	report_def_str = JSON.stringify(report_def);
...
function joinChecked(obj) {
	...
	for( var index in filters_arr) {
		...
		if(filter_row.module_select[filter_row.module_select.selectedIndex].value.indexOf('self_link_') > -1 ) {
			var the_table = document.getElementById('filters');
			var rows = the_table.rows;
			the_table.removeChild(rows[index]);
		}
		else {
			refreshFilterModule(filter_row.module_select,default_filter,index);			
		}	
	}
	...
	for( var index in groups_arr) {
		current_group_id = index;
		var group_row = groups_arr[groups_count_map[index]];
		refreshGroupModule(group_row.module_select,default_group,index);
		refreshGroupColumn(group_row.column_select,default_group,index);
		refreshGroupQualify(group_row.qualify_select,default_group,index);
	}
}
